...
Годограф берётся только до того куска, чья скорость выше потолка самого быстрого
слоя модели: если слоёв задано два, а на ПВ виден и третий, окно второго
кончается реальным удалением перехода, а не дальним удалением съёмки.
...
class Config:
    ...
    def __init__(self):
        ...
        # пропускается:
        #   'degenerate' — окно ставится в границу соседних слоёв и разводится
        #                  до шага округления (spread_empty), скорость берётся
...
def build(pieces: list, cfg: Config) -> tuple:
    ...
    # кусок годографа быстрее самого быстрого слоя модели нам не принадлежит:
    # если слоёв задано два, а на этом ПВ виден и третий, окно второго обрывается
    # там, где скорость ушла выше его потолка, а не на дальнем удалении съёмки
    for x1, x2, t0, v in pieces:
        if v > cfg.layers[-1][2]:
            x_end = min(x_end, x1)
            break
...
def run(cfg: Config, log=print) -> None:
    ...
    for pv, pieces in data:
        ...
        if rms is not None and rms > cfg.bad_rms:
            rough.append((rms, pv))
        # слои без вступлений: скорость у них не измерена, а взята серединой
        # диапазона, поэтому в проверки скоростей они не идут
