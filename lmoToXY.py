...
Границы слоёв в удалениях — то, ради чего всё и считается. Способ подбора
(method), диапазоны слоёв и обращение с пропавшим слоем (missing) описаны в
Config.

Кусок годографа достаётся слою, в чей диапазон попадает его скорость: в окно
второго слоя не должны попадать удаления, где в первых вступлениях ещё первый.
Свобода у границы остаётся только внутри кусков, чья скорость лежит в зазоре
между диапазонами. Годограф берётся до первого куска быстрее потолка последнего
слоя: если слоёв задано два, а на ПВ виден и третий, окно второго кончается
реальным удалением перехода, а не дальним удалением съёмки.

Скорость слоя считается МНК по его окну и в диапазон не зажимается. Выход за
диапазон и упор в его край идут в отчёт: первое значит, что граница проведена
не там, второе — что диапазон узок.
...
class Config:
    ...
    def __init__(self):
        ...
        self.min_offset = 10        # ближнее удаление съёмки: в f1 первый кусок
                                    # начинается с 1 м, ближе окно не ставим
        ...
        # При смежных диапазонах способы дают одни и те же границы: их держит
        # knot_limits. Разойтись им есть где только в зазорах между диапазонами.
        self.method = 'lsq'

        # Что делать со слоем, которого в данных нет. Счёт статики требует
        # одинакового числа слоёв на всех ПВ, поэтому по умолчанию слой не
        # пропускается:
        #   'degenerate' — окно в границе соседних слоёв шириной в шаг
        #                  округления, скорость — середина диапазона;
        #   'all'        — подбор ведётся всеми слоями; окно ненулевое, но его
        #                  границы взяты не из данных;
        #   'neighbours' — ширина окна и скорость с ближайших ПВ, где слой
        #                  выделен: ВЧР меняется плавно, и пропуск чаще от
        #                  редкой пикировки, чем от геологии. Без соседей в
        #                  радиусе — как 'degenerate';
...
def model_lines(p: list, n: int, x_beg: float, x_end: float) -> list:
    ...
    трогает только две соседние прямые, и спуск по ним сходится."""
...
def sq_int(a: float, b: float, u: float, v: float) -> float:
    ...
    Вес 1/x, а не равный по x: ближние слои занимают десятки метров из тысяч, и
    с равным весом МНК жертвует ими ради дальних. С весом 1/x каждая октава
    удалений весит одинаково."""
...
def used_layers(pieces: list, lays: list, missing: str) -> list:
    ...
    Слой, за который не отвечает ни один кусок, в подбор не берём: иначе МНК
    втискивает лишнюю прямую в зазор и выдаёт границу, которой в данных нет.
    В режиме 'all' слои подбираются все, лишь бы их число было одинаковым."""
    if missing == 'all':
        return list(lays)
...
def pad_missing(model: list, lays: list, x_beg: float, x_end: float) -> list:
    ...
    Брать удаления отсутствующему слою неоткуда: схлопываем его окно в границу
    соседей, скорость берём серединой диапазона. Нулевую ширину потом разводит
    spread_empty."""
...
def knot_limits(pieces: list, lays: list, x_beg: float,
    ...
    При смежных диапазонах зазоров нет, и коридор схлопывается до стыка кусков:
    граница встаёт ровно туда, где скорость перешла порог."""
    owned = []
    for x1, x2, t0, v in pieces:
        num = None
        for lay in lays:
...
def window_vels(curve: list, bounds: list, lays: list,
    ...
    границы заданы данными, и ломаная, вытягивая общую невязку, перекашивала бы
    скорости.

    По окну уже min_width скорость не считается вовсе — берётся середина
    диапазона слоя: МНК на таком окне вырождается."""
    lines, vels = [], []
...
def threshold_x(pieces: list, thr: float, x_beg: float, x_end: float) -> float:
    ...
    Ширина перехода — по более короткому из двух: последний кусок в f1 тянется
    до дальнего удаления, и интерполяция между серединами уносила бы границу на
    его середину.
...
def fit_threshold(pieces: list, lays: list, x_beg: float, x_end: float,
    ...
    смежных диапазонов это их общая граница."""
...
def moves(p: list, k: int, s: float, n: int, lines: list) -> list:
    ...
    Без сдвига вдоль прямой граница стоит намертво: двинуть её при неподвижном
    времени в узле значит сразу сломать обе соседние скорости."""
    out = []
    for d in (s, -s):
        q = list(p)
        q[k] += d
        out.append(q)
        if k <= n:                          # k — время в узле. Сдвиг хвоста
            q2 = list(p)                    # меняет скорость ровно одного слоя,
            for j in range(k, n + 1):       # одиночный сдвиг узла — сразу двух
                q2[j] += d
            out.append(q2)
...
def build(pieces: list, cfg: Config) -> tuple:
    ...
    # кусок быстрее потолка последнего слоя нам не принадлежит: окно обрывается
    # там, где скорость ушла выше, а не на дальнем удалении съёмки
    for x1, x2, t0, v in pieces:
        if v > cfg.layers[-1][2]:
            x_end = min(x_end, x1)
            break
...
def nearest_fill(donors: list, xy: tuple, count: int, radius: float):
    ...
    знает сам, неизвестна только протяжённость пропавшего слоя."""
    near = []
...
def spread_empty(model: list, step: int, fills: dict = None) -> list:
    ...
    Окно вида «130 130» Omega не принимает, ей нужно строго max > min. Такое
    выходит и у слоя, которого в данных нет, и у слоя тоньше шага — обе границы
    садятся на один узел сетки.

    Расширить окно на месте нельзя, оно налезет на соседа. Место берётся у
    следующего слоя, там, где вступления узкого слоя и были бы, и всё за ним
    сдвигается вправо; излишек снимается с самого широкого слоя, чтобы модель
    кончалась там же. У последнего слоя место берётся слева, за ним ничего
    нет."""
...
def round_window(x_from: float, x_to: float, step: int, mode: str) -> tuple:
    ...
    На узком слое сжатие вывернет окно наизнанку — там берётся обычное
    округление: окно с краевыми трассами лучше пустого."""
    def near(v):
...
def run(cfg: Config, log=print) -> None:
    ...
    # край, за которым сразу начинается соседний слой, расширять некуда: упор
    # в него значит лишь, что скорость села на границу между слоями
    shared = set()
    for a, b in zip(cfg.layers, cfg.layers[1:]):
        if abs(a[2] - b[1]) < 1e-9:
            shared.add((a[0], 'hi'))
            shared.add((b[0], 'lo'))
    # первый проход — модели по всем ПВ: донором для заполнения может быть
    # только ПВ, у которого модель уже построена
    shots = []
    for pv, pieces in data:
        ...
        if rms is not None and rms > cfg.bad_rms:
            rough.append((rms, pv))
        # у слоёв без вступлений скорость не измерена, а взята серединой
        # диапазона — в проверки скоростей они не идут
    ...
    # первого слоя модель стартует с 1 м из первой строки f1
    off_rows, squeezed = [], []
    ...
    if not rows:
        # пустой прогон файлы не переписывает: обычно это несовпавший SPS, и
        # прежний расчёт затирать нулём нельзя
...
def read_blocks(path: Path, log) -> dict:
    ...
    Номер слоя стоит только в первой строке блока, поэтому конкатенацией файлы
    не склеить — блоки надо разобрать и собрать заново."""
    result, num = {}, None
    with open(path, 'r') as f:
...
def merge_dir(dir_path: str, log=print) -> None:
    ...
    берётся, иначе повторный прогон сложил бы его сам в себя. Строки с
...
class Window(QWidget):
    ...
    сохраняются в QSettings: пути к рабочим дискам длинные."""
    ...
    def show_format(self):
        """Окно разбора файлов. Немодальное — счёт идёт в фоне, модальное
        блокировало бы основное окно. Живёт до конца сессии: группы принадлежат
        ему, и пересоздание их бы разрушило."""
    ...
    def renumber(self):
        ...
        Иначе удаление строки из середины оставляло дыру: слои шли 1, 2, 4, и
        в файлы уходил слой с чужим номером."""
